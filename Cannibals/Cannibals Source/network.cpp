...
void Network::init(const std::vector<unsigned int>& layerSizes, std::mt19937& generator) {
	const unsigned int depth = layerSizes.size();
	matricies.reserve(depth - 1);
...
void Network::compute(const std::vector<float>& input, std::vector<float>& out) {
	...
	for (int i = 0; i < biases.size(); ++i) {
		...
		for (int i = 0; i < buffers[1].size();++i) {
			buffers[1][i] = activation(buffers[1][i]);
		}
	}
...
void Network::copy(const Network& other, std::mt19937 & generator) {
